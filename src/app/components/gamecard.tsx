...
const GameCard: React.FC<props> = ({ index, gameset, item }) => {
    return (
        <div key={index} onClick={() => gameset(item.gameid)} className=" 
    hover:shadow-md hover:shadow-black hover:-translate-x-2 hover:-translate-y-2 duration-150 ease-linear
    flex rounded-tl-[4px] rounded-tr-[4px] flex-col items-center justify-center  h-[322px] w-[250px]  bg-[#181920] bg-opacity-[10%] backdrop-blur-lg">
            {/* img */}
            <div
                style={{ backgroundImage: `url(/assets/${item.img})` }}
                className={`flex 
     hover:cursor-pointer w-full h-[73%] ring-0 ring-white   bg-cover bg-top bg-no-repeat rounded-tl-[4px] rounded-tr-[4px]`}></div>
            {/* desc */}
