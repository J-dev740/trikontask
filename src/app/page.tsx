            ...
            <div className="flex flex-row w-[90%] h-fit justify-center items-center gap-[40px] text-white font-Mont font-medium text-[14px] leading-[17px] ">
              {
                Object.values(categories).map((val,index)=>{
                  return (
                    <div key={index} className={`font-Mont w-fit text-center mx-auto font-bold text-[16px] hover:cursor-pointer ${category==val?'bg-[#F68E5F] rounded-[48px] px-[16px] py-[6px]':'bg-transparent'}`} onClick={()=>setCategory(val)}>{val}</div>
                  )
                })
              }
